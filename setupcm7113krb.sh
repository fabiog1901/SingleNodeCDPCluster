...
# Current version of this script is based on centos7.  With mods it should work for later RHEL versions.  Wouldn't need the python 3.8 build section 
start_dir=$PWD
...
# Still need to fix the create_cluster_krb.py to use archive CM repo URL with the download user:pass and latest CM API version v54
...
#sed -i "s/archive\.cloudera\.com\/cm7\/7.4.4/$DOWNLOADUSER:$DOWNLOADPASS@archive\.cloudera\.com\/p\/cm7\/7.11.3.0/" scripts/create_cluster_krb.py
...
sed -i "s/archive\.cloudera\.com/$DOWNLOADUSER:$DOWNLOADPASS@archive\.cloudera.com/" $TEMPLATE
